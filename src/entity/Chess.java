...
class Chess {
    ...
    Chess(int size){
        ...
        addPiece(new Piece(InfoConstant.WHITE,size / 2, size / 2));
    ...
    ArrayList<Position> getOptions(int color){ //获得某个颜色的可选合法位置
        ArrayList<Position> positons = new ArrayList<>();
        for(int i = 0; i < dimension; i++){
            for(int j = 0; j < dimension; j++){
                if(checkAvailab(i,j,color,false) > 0){
                    positons.add(new Position(i,j));
                }
            }
        }
        return positons;
    ...
    private int checkAvailab(int x, int y, int color, boolean isMove){ //返回某个位置放置棋子后需要翻转的棋子数目
        ...
        for(int i = 0; i < 8; i++){
            ...
            while (x + (index + 1) * direction[i][0] >= 0 && x + (index + 1) * direction[i][0] < dimension
                    ...
                    && pieces[x + index * direction[i][0]][y + index * direction[i][1]].getColor() == Util.getAnotherColor(color)){
    ...
    int checkAvailab(Position positon, int color, boolean isMove){
    ...
    void move(Position positon, int color){
